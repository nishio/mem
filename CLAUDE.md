...
- **型エラーがあるとVercelのデプロイが失敗し、本番サイトが壊れる**
- ローカルで事前に確認することでデプロイ失敗を防げる
- **2025-11-28の教訓**: ES5互換性のないコード（スプレッド演算子など）がpushされ、ビルドが失敗した

**チェックリスト**:
- [ ] `yarn tsc --noEmit` が成功することを確認（必須）
- [ ] `yarn build` が成功することを確認（推奨）
- [ ] TypeScriptの型エラーがないことを確認
- [ ] eslint/prettier の警告を確認（必要に応じて修正）
- [ ] コミット・プッシュ

**自動化**:
- Git pre-commit hookが設定されており、コミット時に自動的に型チェックが実行されます
- エラーがある場合はコミットが中断されます
- 緊急時のみ `git commit --no-verify` でスキップ可能（非推奨）
...
**vt_config.jsonの`illusts`配列の順序は「追加順」を表す設計になっています。**

**ルール:**
- **ID**: ただの識別子（URL用のkey）、順序の意味は一切ない
- **時間順（追加順）**: vt_config.jsonの配列順で表現する
- **「最新順」**: 「追加順の逆順」として定義する（`[...illusts].reverse()`）

**実装への影響:**
- `/latest`ページ: 配列順を逆順にして表示（`illusts.reverse()`）
- Prev/Nextリンク: 配列順のインデックスで前後に移動
- 新規追加: 必ず配列の最後に追加すること
- **絶対にIDでソートしてはいけない**（追加順が壊れる）

**理由:**
- IDは欠番を埋める仕組みがあるため、ID順 ≠ 追加順
- 配列順を使うことで、シンプルかつ破壊的変更なしで実装できる
